...
# todo: change here !!!!!
#base_path = os.path.join('/data/user/runzeli/FD_test_cobalt/FD_test.py','')
...
def main(v_dir, p, E, pos):
    ...
    while 1:
        ...
        for i in range(101):
            pos_1 = pos_1 - i * step * dir

            if pos_1[0] <= 500 and pos_1[0] >= -500 and pos_1[1] <= 500 and pos_1[1] >= -500 and pos_1[2] <= 6369500 and pos_1[2] >= 6368500:
                return 0
        """-------------------------------------------------------"""
...
"""----------------------initializer--------------------------"""
def try_this(theta_index, phi_index):

    theta_pos, phi_pos, azimuth_dir, zenith_dir, energy = get_rand_ini(theta_index, phi_index)

    pos = 6371000 * changecord(theta_pos, phi_pos)
    ...
    check = main(v_dir, p, energy, pos)


    return check


def iteration(theta_ind, phi_ind, f):
    counter = 0
    theta_index = theta_ind
    phi_index = phi_ind
    i = 1
    while 1:
        check = try_this(theta_index, phi_index)

        if check == 0:
            counter = counter + 1

        print(str(phi_index) + " " + str(i) + " " + str(counter))
    ...
    # f = open("result_" + str(jth) + ".txt", "w")
    f.write(str(phi_index) + " " + str(counter) + " " + str(i) + "\n")
    f.flush()


parser = argparse.ArgumentParser(description='specify the range of phi index, from 0 for 0-4 to 3 for 15-19, and theta from 0 to 4')
parser.add_argument('theta_index', metavar='N', type=int, nargs='+', help='index from 0 to 4')
parser.add_argument('phi_index', metavar='N', type=int, nargs='+', help='index from 0 to 3')


args = parser.parse_args()
theta_index = int(args.theta_index[0])
phi_index = int(args.phi_index[0])

if phi_index == 0:
    f = open("result_" + str(phi_index) + ".txt", "w")
    for i in [1, 4]:
        iteration(theta_index, i, f)
    f.close()
